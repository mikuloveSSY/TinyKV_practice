...
> **注意**：TinyKV 使用了 badger 的一个 fork 版本（`github.com/Connor1996/badger`），包含一些修复，**不要**使用官方版本（`github.com/dgraph-io/badger`）。
...
**所以 P1 的核心规则就是**：你所有的读写操作都必须走 `engine_util` 提供的方法。不要直接用 badger 的 `txn.Set`、`txn.Get`——因为 badger 不认识列族前缀，直接调会绕过前缀机制，导致数据错乱。更多信息可阅读 `kv/util/engine_util/doc.go`。
...
**你需要做的事**：只写 `RawGet/Put/Delete/Scan` 函数的内容逻辑（从 request 取参数，调 storage，填 response）。proto 文件、序列化、网络传输这些都不用管。如果确实需要修改 proto 文件，改完后运行 `make proto` 重新生成 Go 代码。
...
>
> **重要提醒**：不要忘记对 `badger.Txn` 调用 `Discard()`（在 `Close()` 方法里），并在丢弃事务之前关闭所有迭代器（`defer iter.Close()`），否则会导致内存泄漏。
